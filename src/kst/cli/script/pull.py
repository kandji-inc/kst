...
def pull_scripts(
    ...
    try:
        git.commit_all_changes(cd_path=repo, message="Before pulling scripts from Kandji", scope=repo, skip_git=no_git)
    ...
    try:
        git.commit_all_changes(cd_path=repo, message="After pulling scripts from Kandji", scope=repo, skip_git=no_git)
